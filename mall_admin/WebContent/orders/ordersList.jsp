...
<body>
	...
	<%
		...
		// list 생성	
		ArrayList<OrdersAndEbookAndClient> list = OrdersDao.ordersList(rowPerPage, beginRow);
	%>
            ...
            <div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body">
                                <h5 class="card-title">Orders Datatable</h5>
                                <div class="table-responsive">
									...
									<table id="zero_config" class="table table-striped table-bordered">
											...
											<tr>
												<th><span style="font-weight:bold">ordersNo</span></th>
												<th><span style="font-weight:bold">ebookNo</span></th>
												<th><span style="font-weight:bold">clientNo</span></th>
												<th><span style="font-weight:bold">ordersState</span></th>
												<th><span style="font-weight:bold">주문상태 수정</span></th>
												<th><span style="font-weight:bold">ordersDate</span></th>
												<th><span style="font-weight:bold">ebookTitle</span></th>
												<th><span style="font-weight:bold">clientMail</span></th>
											</tr>
										...
										<tbody>
											...
											%>
													<tr>
														<td><%=oec.orders.getOrdersNo()%></td>
														<td><%=oec.orders.getEbookNo()%></td>
														<td><%=oec.orders.getClientNo()%></td>
																			
														<td>
														<form action="<%=request.getContextPath()%>/orders/updateOrdersStateAction.jsp?" method="post">
															<input type="hidden" name="ordersNo" value="<%=oec.orders.getOrdersNo()%>">
															<select name="ordersState">
															<%
															String[] state = {"주문완료","주문취소"};
																for(String s : state){
																	if(s.equals(oec.orders.getOrdersState())) {
																%>
														...
														<td><%=oec.orders.getOrdersDate().substring(0,10)%></td>
														
														<td><a href="<%=request.getContextPath()%>/ebook/ebookOne.jsp?ebookNo=<%=oec.orders.getEbookNo()%>"><%=oec.ebook.getEbookTitle()%></a></td>
														<td><%=oec.client.getClientMail()%></td>
													</tr>
											<%
												}
											%>
									...
									<%
										...
										if(currentPage != 1){
									%>
											<a href="<%=request.getContextPath()%>/orders/ordersList.jsp?currentPage=<%=currentPage-1%>&rowPerPage=<%=rowPerPage%>"><button type="button" class="btn btn-outline-secondary">Previous</button></a>
									<%
										}
									%>
									<%
									...
									%>
											<a href="<%=request.getContextPath()%>/orders/ordersList.jsp?currentPage=<%=currentPage+1%>&rowPerPage=<%=rowPerPage%>"><button type="button" class="btn btn-outline-secondary">Next</button></a>
									<%
										}
									%>
