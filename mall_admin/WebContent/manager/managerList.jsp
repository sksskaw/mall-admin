...
<%
	...
	//세션 정보가 있으면서 관리자 레벨이 2이상이여야 리스트를 볼 수 있다. 아니면 adminIndex페이지로 돌아가기
	...
	ArrayList<Manager> list = ManagerDao.selectManagerList(rowPerPage, beginRow, searchWord);
%>
            ...
            <div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body">
                                <h5 class="card-title">Manager Datatable</h5>
                                <div class="table-responsive">
									<table id="zero_config" class="table table-striped table-bordered">
											<tr>
												<th><span style="font-weight:bold">managerNo</span></th>
												<th><span style="font-weight:bold">managerId</span></th>
												<th><span style="font-weight:bold">managerName</span></th>
												<th><span style="font-weight:bold">managerDate</span></th>
												<th><span style="font-weight:bold">Level 수정</span></th>
												<th><span style="font-weight:bold">삭제</span></th>
											</tr>
										<tbody>
											<%
												for(Manager m : list){
											%>
												<tr>
													...
													<td>
														<a href="<%=request.getContextPath()%>/manager/deleteManagerAction.jsp?managerNo=<%=m.getManagerNo()%>"><button type="button" class="btn btn-danger btn-sm">삭제</button></a>
													</td>
												</tr>
											<%		
												}
									...
									<%
										...
										if(currentPage != 1){
									%>
											<a href="<%=request.getContextPath()%>/manager/managerList.jsp?currentPage=<%=currentPage-1%>&rowPerPage=<%=rowPerPage%>&searchWord=<%=searchWord%>"><button type="button" class="btn btn-outline-secondary">Previous</button></a>
									<%
										}
									%>
									<%
										//마지막 페이지에는 > 버튼이 보이지 않게
										if(currentPage < (ManagerDao.totalCount(searchWord)/rowPerPage) + 1){ //나머지 부분도 출력하기 위한 +1
									%>
											<a href="<%=request.getContextPath()%>/manager/managerList.jsp?currentPage=<%=currentPage+1%>&rowPerPage=<%=rowPerPage%>&searchWord=<%=searchWord%>"><button type="button" class="btn btn-outline-secondary">Next</button></a>
									<%
										}
									%>
									<!-- 관리자 이름 검색 폼 -->
